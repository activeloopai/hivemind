...
var { log: wikiLog } = makeWikiLogger(join14(homedir9(), ".claude", "hooks"));
async function main() {
  if (process.env.HIVEMIND_WIKI_WORKER === "1")
    return;
  const input = await readStdin();
  spawnDetachedNodeWorker(join14(__bundleDir, "graph-deps-worker.js"));
  ...
  if (embeddingsDisabled()) {
    ...
    try {
      const daemonEntry = join14(__bundleDir, "embeddings", "embed-daemon.js");
